...
$(".contact_btn").on('click', function () {
    ...
    if(proceed == "false")
    {
        output = '<div class="alert-danger" style="padding:10px 15px; margin-bottom:30px;">Merci de remplir tous les champs.</div>';
    ...
    var json = JSON.stringify(frm);
    $("#result").hide()
    $.ajax({
        type: 'POST',
        url: "https://sendmailgithubpagesjokrvsep-first.functions.fnc.fr-par.scw.cloud/",
        data: json,
        dataType: 'json',
        ...
        error: function (e) {
            var output = '<div class="alert-danger" style="padding:10px 15px; margin-bottom:30px;">Merci de remplir tous les champs.</div>';
            $("#result").hide().html(output).slideDown();
        }
    });
